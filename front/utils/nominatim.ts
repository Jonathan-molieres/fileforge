...
export default class NominatimJS {
    ...
    public static async search(rawParams: ISearchParams): Promise<NominatimSearchResult[]> {
        const params: Record<string, any> = NominatimJS.normalizeParams({
            ...rawParams,
            addressdetails: 1,
        })
        const countryCodes =
            params.countrycodes ||
            (params.countryCodesArray ? params.countryCodesArray.join(',') : undefined)
    ...
    public static async lookup(
        ...
        const params: Record<string, any> = NominatimJS.normalizeParams(rawParams)
